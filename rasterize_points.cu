	...
	const bool debug)
{
  ...
  torch::Tensor out_alpha = torch::full({0, H, W}, 0.0, float_opts);
  float* out_invdepthptr = nullptr;
  float* out_alphaptr = nullptr;

  out_invdepth = torch::full({1, H, W}, 0.0, float_opts).contiguous();
  out_invdepthptr = out_invdepth.data<float>();
  out_alpha = torch::full({1, H, W}, 0.0, float_opts).contiguous();
  out_alphaptr = out_alpha.data<float>();
  ...
  return std::make_tuple(rendered, out_color, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth);
}
